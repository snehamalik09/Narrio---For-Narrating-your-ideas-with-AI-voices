...
const PodcastDetailPlayer = ({ podcastThumbnail, podcastTitle, audioDuration, audioUrl, podcastID, authorID, authorName, authorImgUrl }: {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-6 md:gap-12 py-5 md:py-8 justify-center relative">
      ...
      <div className="flex-1 flex flex-col md:gap-1 gap-2 pt-4 md:pt-7 text-center md:text-left relative">
    ...
    flex justify-center items-center md:absolute md:bottom-5 md:left-0 gap-2 cursor-pointer
    w-full md:w-60 py-2 px-6 rounded-full shadow-lg
  ...
  `}
        >
          <Image
            ...
            height={20}
          />
          <p className="tracking-wide truncate">{isPlaying && pID === podcastID ? "Pause" : "Play Podcast"}</p>
        </div>

      </div>

      {isOwner && (
        <div className="absolute top-5 md:top-8 right-2 z-50">
